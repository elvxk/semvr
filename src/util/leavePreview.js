...
export const leavePreview = async (interaction) => {
  ...
  const serverWelcomer = await prisma.leave.findFirst({
    where: { server_id: interaction.guild.id },
  });
  if (!serverWelcomer)
    return await interaction.reply(
      "`🍻` No leave set up\n" + "`🍻` Use `/leave setting` to set leave",
    );

  if (!serverWelcomer.enabled)
    return await interaction.reply(
      "`🍻` leave is disabled\n" + "`🍻` Use `/leave toggle` to enable leave",
    );
